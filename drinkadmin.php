...
<?php require_once("_nav.php"); ?>
    <main>
        ...
        <form action="drinkadmin.php" method="POST">
            ...
            <div class="user_data"><?=$id?>$nbsp;$nbsp;$row['drinkname']<br>$row['description']<br>$row['rating']</div>
            ...
            <select name="alcoholic">
                <option value="0" <? if($row['alcoholic']==0){echo "selected";} ?> >Not alcoholic</option>
                <option value="1" <? if($row['alcoholic']==1){echo "selected";} ?> >contains alcohol</option>
            </select>
            <label for="ingredients">Ingredients (Separate in rows)</label>
            <textarea name="ingredients" rows="10"<?=$row['ingredients']?>></textarea>
            <label for="recipe">How do you make the drink?</label>
            <textarea name="recipe" id="" rows="10" <?=$row['recipe']?>></textarea>
